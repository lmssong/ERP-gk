...
*/
@Component({
  selector: 'page-gpmsMonth',
  templateUrl: 'gpmsMonth.html'
})
export class GpmsMonthPage {
  ...
  constructor(public navCtrl: NavController, public navView: ViewController, public modalCtrl: ModalController, public toastCtrl: ToastController, public gpmsMonthProvider: GpmsMonthProviders) {
  ...
  ionViewDidEnter() {
    //加载客户
    this.gpmsMonthProvider.loadGetCustomerList().then((data: any[]) => {
  ...
  loadProductList(startDate, customerName) {
    this.gpmsMonthProvider.loadGpmsMonthList(this.startDate, this.customerName).then((data: any[]) => {
